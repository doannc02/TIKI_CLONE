...
import { BLACK, GREEN } from '@/components/layouts/WrapLayout/ModeTheme/colors'
...
const CustomCompareTable = ({
  ...
  const getFieldValue = (
    ...
    if (typeof value === 'string' && nameField === 'scopeType') {
      const render = scopeCustomerTypeSelect.find(item => item.value === value )
  ...
  const getFieldValueTT = (
    ...
    return (
        <CurrencyFormatCustom color={`${nameField in findDiffTT && isPaymentChange ? 'red' : ''}`} amount={value??0} />
    )
  }
  return (
    <>
...
const checkFieldDifference = (
  ...
  if(fieldName === 'taxNames'){
    ...
    if (fieldName !== 'taxNames' && mainLine[fieldName] !== changeLine[fieldName]) {  
      differences.push({
        index,
        value: changeLine[fieldName],
        fieldName,
...
const findDifferences = (
  ...
  const tempArr = mainLines.length > changeLines.length && changeLines.length === 0 ? mainLines : changeLines
  tempArr.forEach((mainLine, index) => {
    const changeLine = changeLines.find(
      (line) => line.id === mainLine.id
    )
    if (!changeLine || mainLines.length > changeLines.length) {
      ...
      if(mainLines[index]){
      console.log(mainLines[index], "mainLine")
        differences.push({
          index,
          value: '',
          fieldName: "field",
      ...
      checkFieldDifference('quantity', mainLines[index], changeLine, differences, index)
...
export const TableCompareInfInvoice = ({
  ...
  const render = (item: any, isColorRed: boolean) => {
    ...
    let color = isColorRed ? 'red' : ''
        if (typeof item === 'object' && item?.fieldName === '' && item?.value  === 'REMOVED') {
      return <Typography color='red'>Đã xóa</Typography>
    }
    return (
      <>{item ? <TableBody sx={{ '& .MuiTableRow-root': { borderBottom: 'none' } }}>
      ...
      <TableRow sx={{ borderBottom: 'none' }}>
        ...
        <TableCell sx={{ borderBottom: 'none' }} color={color}>
          <Typography color={color}>
            {<CurrencyFormatCustom amount={item?.unitPrice} color={color}/>}{' '}
          </Typography>
        </TableCell>
      </TableRow>
      <TableRow sx={{ borderBottom: 'none' }}>
    ...
    </TableBody> : <Typography>N/A</Typography>}</>
