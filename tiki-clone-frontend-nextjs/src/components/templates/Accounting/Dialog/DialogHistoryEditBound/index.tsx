...
const DialogHisUpdateList = ({
  ...
  const { typePath, paymentType, paymentMethod, balanceTypePath } = useCheckPath()
  ...
  return (
    <DialogCustom title='' onClose={hideDialog} width={1010}>
      <Box className='flex justify-center px-25 m-auto align-middle text-center'>
        ...
        >
          {
             historyType !== "PAYMENT" ? `Lịch sử chỉnh sửa Hóa đơn ${code}`: ""
          }     
          {
            historyType === "PAYMENT" && paymentType === "INBOUND" && `Lịch sử chỉnh sửa Phiếu thu ${code}`
          }
          {
            historyType === "PAYMENT" && paymentType === "OUTBOUND" && `Lịch sử chỉnh sửa Phiếu chi ${code}`
          }
        </Typography>
      </Box>
      <Box className='mx-20'>
