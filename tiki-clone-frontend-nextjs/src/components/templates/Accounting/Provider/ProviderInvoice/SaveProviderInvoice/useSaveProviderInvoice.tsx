...
import { getAccountNames, useQueryGetAccountNames } from '@/service/accounting/account/getNames'
  ...
  const invoiceLinesTableData = (fields ?? []).map((item, index) => {
    if (actionType === 'VIEW')
      return {
        ...
        unitPrice: <CurrencyFormatCustom variant='body1' amount={item.unitPrice} />,
        taxIds: item.taxIds
          .map((ele) => {
            if (taxSelect) {
...
export default useSaveProviderInvoice
